...
const FeatureCard = ({ title, description, children, delay }: { title: string, description: string, children: React.ReactNode, delay: number }) => (
    <motion.div
        ...
        className="bg-white rounded-[24px] p-8 md:p-10 shadow-sm flex flex-col gap-8 hover:shadow-md transition-shadow duration-300"
    >
            ...
            <div className="max-w-7xl mx-auto px-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <FeatureCard
                        ...
                        delay={0.1}
                    >
                        <div className="w-full max-w-[240px] bg-white rounded-xl p-4 shadow-sm relative">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex-grow">
                                    ...
                                    <div className="text-[10px] text-gray-400 mt-0.5">45 KB / 145 KB</div>
                                </div>
                            </div>
                    ...
                    >
                        <div className="relative h-[120px] w-full flex justify-center items-center">
                            {[
                                { name: 'CAD', color: 'bg-purple-100 text-purple-600', x: 'translate-x-[45px] rotate-6' },
                                { name: 'GBP', color: 'bg-emerald-100 text-emerald-600', x: 'translate-x-[15px] rotate-2' },
                                { name: 'EUR', color: 'bg-rose-100 text-rose-600', x: '-translate-x-[15px] -rotate-2' },
                                { name: 'USD', color: 'bg-blue-100 text-blue-600', x: '-translate-x-[45px] -rotate-6' },
                            ].reverse().map((card, i) => (
                                <div
                                    key={i}
                                    className={`absolute w-16 h-24 ${card.color} rounded-lg shadow-sm font-bold text-xs flex flex-col justify-between p-2 transform transition-transform ${card.x}`}
                                >
                                    <span>{card.name}</span>
                                    <div className="w-3 h-3 border-2 border-current rounded-sm opacity-30 self-end" />
                                </div>
                            ))}
                        </div>
                    </FeatureCard>
                    ...
                    <FeatureCard
                        ...
                        delay={0.3}
                    >
                        <div className="w-full max-w-[260px] bg-white rounded-xl overflow-hidden shadow-sm">
                            <div className="bg-gray-50/50 px-4 py-2 flex items-center justify-between">
                                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">AI Collections Agent</span>
                            ...
                            <div className="p-4 space-y-4">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[10px] text-gray-500">Day 1 Timestamp</span>
                                    </div>
                                    <div className="flex items-center gap-1 text-blue-600">
                                        <CheckCircle2 size={12} />
                                        <span className="text-[10px] font-bold">2 invoices paid</span>
                                    </div>
                                </div>
                                <div className="pt-3 flex items-center justify-between text-gray-400">
